...
// Same "server-side backstop" reasoning as MAX_PHOTO_DATA_URL_LENGTH above,
// and MAX_KEYWORD_LENGTH/MAX_KEYWORDS_PER_CHECK in ResumeController —
// generatedSummary/generatedBullets were previously only ever set by the
// bounded rule-based generator or a trusted admin edit; now that a
// subscriber can PUT them directly (see the "Generated Summary & Bullets"
// panel on Edit Resume), a client-side textarea limit alone isn't a
// guarantee against a malformed or hostile request. Limits are generous —
// several paragraphs of summary, dozens of full-sentence bullets — well
// past anything a real resume summary/bullet list would need.
const MAX_GENERATED_SUMMARY_LENGTH = 4_000;
const MAX_GENERATED_BULLETS_COUNT = 60;
const MAX_GENERATED_BULLET_LENGTH = 500;

export class GeneratedContentTooLargeError extends Error {}
...
export interface CreateResumeRequest {
  ...
  /** "Generate AI cover letter" checkbox — silently coerced to false server-side when templateKey isn't a Premium-tier template. */
...
export class ResumeService {
  ...
  async update(userId: string, resumeId: string, input: UpdateResumeInput): Promise<Resume> {
    ...
    // stays "live" — unless the subscriber has hand-edited the Summary/
    // bullets themselves (summaryManuallyEdited), in which case that manual
    // edit should survive an unrelated Work Experience/profession/name/title
    // change instead of being silently clobbered. Sending
    // summaryManuallyEdited: false is the explicit "Reset to auto-generated"
    // action, which forces one regeneration pass right now regardless of
    // whether anything else in this request actually changed.
    ...
    const contentInputsChanged = !!(input.answers || input.achievements || professionChanged || nameChanged || titleChanged);
    const resetToAutoGenerated = input.summaryManuallyEdited === false;
    let summaryManuallyEdited = resetToAutoGenerated ? false : input.summaryManuallyEdited ?? existing.summaryManuallyEdited;

    let generatedSummary = input.generatedSummary;
    let generatedBullets = input.generatedBullets;
    if (resetToAutoGenerated || (contentInputsChanged && !summaryManuallyEdited)) {
      ...
      generatedSummary = generated.summary;
      generatedBullets = generated.bullets;
    } else if (input.generatedSummary !== undefined || input.generatedBullets !== undefined) {
      // A direct edit to the summary/bullets fields themselves — with no
      // reset requested — is the subscriber hand-editing them; mark it so
      // it isn't silently regenerated over next time.
