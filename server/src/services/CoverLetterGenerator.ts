 ...
 * deterministic template logic" approach as ContentGenerator.ts (see that
 * file's header comment for why: no LLM/network call wired into resume
 * generation, which runs synchronously on every save). Only ever invoked
 * for resumes on a Premium-tier template with the "Generate AI cover
 * letter" checkbox on — see ResumeService.create/update.
 */
 ...
 * heuristic as ResumePreview.tsx's sortByDateRange on the client, kept
 * simple here since a cover letter only ever calls out one role.
 */
