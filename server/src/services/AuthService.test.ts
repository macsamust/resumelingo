...
/**
 * Mocks just the methods AuthService actually calls, rather than a full
 * UserRepository — this class talks to Postgres via `pg`, which has no
 * business running inside a unit test. See worker/src/services/
 * AuthService.test.ts for the D1-backend equivalent of every case here.
 */
...
function makeTokensMock() {
  // Typed with a rest-args implementation (rather than a specific
  // zero/one-arg signature) so Vitest doesn't narrow the mock's inferred
  // call-args type — this mock's calls get asserted with toHaveBeenCalledWith
  // below, which needs to accept whatever arguments the real call site passes.
  return { sign: vi.fn((..._args: unknown[]) => "signed-token"), verify: vi.fn() } as unknown as TokenService;
}
