 ...
 * these as plain functions rather than round-tripping through the full
 * ResumeService class (which would require mocking five repositories/
 * generators just to reach a policy check). See worker/src/services/
 * ResumeService.test.ts for the same set of cases against the D1 backend,
 * since these gates are meant to behave identically on both.
 */
