 ...
 * fabricated stat. A clause is simply omitted (not guessed at) when the
 * underlying data isn't there.
 *
 * Built from five things, in order: years of experience (computed from the
 * work history's date range), focus area (the resume's profession label),
 * a quantified achievement (pulled verbatim from an achievement's "result"
 * field, or a bullet's "resulting in ..." clause, whichever has a number in
 * it), technical expertise (the same extractKeywords() skill list already
 * shown lower in the card), and career direction (the resume's own target
 * title).
 */
...
/** "a" or "an", based on the focus area's leading sound — good enough for the profession labels this app actually has (e.g. "an Executive", "a Nurse"). */
