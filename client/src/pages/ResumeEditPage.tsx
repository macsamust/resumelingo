...
export function ResumeEditPage() {
  ...
  /** Discards the manual edit and regenerates from profession + answers + achievements, same as before this feature existed. */
  const resetSummaryToAutoGenerated = async () => {
    if (!id) return;
    setSummaryError(null);
    setSummarySaving(true);
    try {
      ...
      setSummaryError(err instanceof ApiError ? err.message : "Something went wrong resetting your summary.");
    } finally {
      setSummarySaving(false);
    }
  };
      ...
      <form id="resume-edit-form" onSubmit={onSubmit} className="builder-grid" onBlur={handleFormBlur}>
        <div className="builder-panel">
          ...
          <CollapsibleSection title="Generated Summary & Bullets" forceOpen={forceOpen} defaultOpen={false}>
            <p className="hero-note" style={{ marginBottom: 16 }}>
              Your Objective/Summary/Profile text and top bullets are generated automatically from your profession,
              answers, and achievements above. Edit them directly here if you'd like different wording — once saved,
              they'll stay exactly as you write them and won't get regenerated when you update Work Experience,
              Achievements, or the fields above, until you reset them back to auto-generated.
            </p>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              {summaryManuallyEdited && (
                <button type="button" className="btn btn-ghost" onClick={resetSummaryToAutoGenerated} disabled={summarySaving}>
